...
Homebrew로 앱을 설치한 뒤, CLI도 별도로 PATH에 등록해야 합니다. cmux 앱 안에서 `Cmd+Shift+P`를 눌러 Command Palette를 열고 **"Shell Command: Install 'cmux' in PATH"**를 실행하면 됩니다.
...
> ⚠️ `/Applications/cmux.app/Contents/MacOS/cmux`는 앱 바이너리입니다. CLI는 `Contents/Resources/bin/cmux`에 있으므로 심링크 경로를 혼동하지 않도록 주의하세요.

### 소켓 접근 권한

Raycast 등 외부 프로세스에서 cmux CLI를 사용하려면 소켓 접근 권한을 열어야 합니다. `~/.config/cmux/settings.json`에서 다음을 설정합니다:
