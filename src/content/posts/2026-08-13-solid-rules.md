...
코딩 에이전트들은 기존 코드를 보고 새로운 코드를 작성합니다. 그래서 Legacy 위에 코드를 쌓으면 코딩 에이전트들은 Legacy 에서 허용한 무너진 설계 위로 더더욱 창의력을 발휘하여 와르르멘션을 구축합니다. 그렇기 때문에 좋은 설계를 유지하는 것은 AI 시대를 살아가는 우리에겐 더욱 중요해졌습니다. 그래서 좋은 품질의 코드를 관리하기 위해 인간 에이전트들은 AI가 작성한 코드를 리뷰하는데 점점 더 많은 시간을 보내게 되었습니다. 이 시간을 어떻게 줄일 수 있을지에 대한 한가지 방법론 이며 이 글을 통해 휴먼 병목을 조금이나마 함께 해소할 수 있기를 기대합니다.
